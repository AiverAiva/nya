...
class guild(commands.Cog):
  ...
  @discord.slash_command(name="guild",
                         description="Check the guild infomation.")
  async def guild(self, ctx: discord.ApplicationContext,
                  guild: Option(str, "The name or prefix of the guild.")):
    start_time = time.time()
    guildData = await requests.getGuild(guild)

    if guildData == "":
      return await ctx.respond(embed=embeds.embedError(
          f'The guild `{guild}` is unknown. Please check if there is any uppercase or whether the tag is correct.'
      ))
    ...
    paginator = pages.Paginator(pages=page_groups,
                                show_menu=True,
                                loop_pages=True,
                                menu_placeholder="Click here to switch tab")
